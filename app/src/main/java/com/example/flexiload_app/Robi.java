...
public class Robi extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setTitle("Robi Offer");
        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getList=new ArrayList<>();
        getDataAdapter1=new Offer_Adapter(getList);
        firebaseFirestore=FirebaseFirestore.getInstance();
        documentReference=firebaseFirestore.collection("Robi").document();
    ...
    private void reciveData() {
        firebaseFirestore.collection("Robi").addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override

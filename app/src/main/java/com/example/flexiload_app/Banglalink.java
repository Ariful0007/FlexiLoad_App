...
public class Banglalink extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setTitle("Banglalink Offer");
        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getList=new ArrayList<>();
        getDataAdapter1=new Offer_Adapter(getList);
        firebaseFirestore=FirebaseFirestore.getInstance();
        documentReference=firebaseFirestore.collection("Banglalink").document();
    ...
    private void reciveData() {
        firebaseFirestore.collection("Banglalink").addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override
